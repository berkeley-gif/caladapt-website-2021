...
<g class='axis x-axis'>
  {#each tickVals as tick, i}
    <g class='tick' transform='translate({$xScale(tick)},{$height + 10})'>
      ...
      <text y='16' text-anchor='{textAnchor(i)}' style="font-size:12px;fill:#666">{formatTick(tick)}</text>
    </g>
  {/each}
  {#if baseline}
    <path class="baseline" d={`M -${$padding.left} ${$height + 10} L ${$width} ${$height + 10}`} style="stroke:#aaa;"></path>
  {/if}
</g>