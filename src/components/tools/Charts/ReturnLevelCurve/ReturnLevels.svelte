...
<g class="rl-group" transform="translate({$padding.left}, 0)">
  {#each data as group}
    <g
      class="{group.timestep}"
      transform="translate({$rScale(group.timestep)},0)"
    >
      {#each group.data as series}
        <SeriesGroup on:mousemove on:mouseover series="{series}" />
      {/each}
    </g>
  {/each}
</g>
